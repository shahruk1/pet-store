...
//		String keyword="gana";
	...
	public String admindeleteUser(@PathVariable String email) {
		...
		if(user.getEmail()!=null) {
			List<Booking> book=user.getBooking();
			for(Booking b:book)
			{
			b.getEvent().getBooking().remove(b);
			b.getHotel().getBooking().remove(b);
			
			b.getCatering().getBooking().remove(b);
	...
	public String admindeleteHotel(@PathVariable int id)
	{
		...
		if(hotel.getHotelName()!=null) {
			List<Booking> book=hotel.getBooking();
			for(Booking b:book)
			{
			b.getEvent().getBooking().remove(b);
		
			b.getUser().getBooking().remove(b);
			b.getCatering().getBooking().remove(b);
	...
	public String savehotel(@RequestParam("subadmin") String role1,@RequestParam("superadmin") String role2,@RequestParam("hotelName") String hotelname,@RequestParam("hotelDesc") String hotelDesc,@RequestParam("location") String hotelLoc,@RequestParam("price") int hotelPrice,@RequestParam("Rate") int Rating,@RequestParam("hotelImg1") MultipartFile file) {
		hotelservice.savehoteltoDB(file, hotelname, hotelDesc, hotelLoc, hotelPrice,Rating);
	...
	public String admindeleteCater(@PathVariable int id)
	{
		...
		if(c.getCatername()!=null) {
			List<Booking> book=c.getBooking();
			for(Booking b:book)
			{
			b.getEvent().getBooking().remove(b);
			b.getHotel().getBooking().remove(b);
	...
	public String saveCater(@RequestParam("subadmin") String role1,@RequestParam("superadmin") String role2,@RequestParam("catername") String catername,@RequestParam("cater_desc") String caterDesc,@RequestParam("cater_price") int caterprice,@RequestParam("cater_img") MultipartFile file) {
		caterservice.savecatertoDB(file, catername, caterDesc,  caterprice);
		if(role1.equals("subadmin")&& role2.equals("not"))
		{
...
//	}
	...
	//Event Table
		...
		public String admindeleteEvent(@PathVariable int id ) {
			...
			System.out.println(v);
			List<Booking> book=v.getBooking();
			for(Booking b:book)
			{
	...
	public String saveevent(@RequestParam("subadmin") String role1,@RequestParam("superadmin") String role2,@RequestParam("eventname") String eventname,@RequestParam("event_desc") String eventDesc,@RequestParam("event_img") MultipartFile file,@RequestParam("event_price") int eventprice) {
		eventservice.saveeventtoDB(file, eventname,eventDesc,eventprice);
			if(role1.equals("subadmin")&& role2.equals("not"))
			{
